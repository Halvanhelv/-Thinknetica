...
production:
  ...
  pool: 20  # позволяет одно подключение к базе данных использовать n  количество раз, полезно что бы не подключаться постоянно, экономит ресурсы, подключением можно делиться только в рамках одного экземпляра Rails приложения, общее количество подключений по умолчанию ограничено до 100, если будет больше то база перестанет отвечать то база перестанет отвечать и скажет что нет доступных подключений
```
